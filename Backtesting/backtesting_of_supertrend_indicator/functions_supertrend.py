...
def plot_signals_with_supertrend(df, entry, exit):
    '''

    :param df:DataFrame containing the price data.
    :param entry:List of entry points for trades.
    :param exit:List of exit points for trades.
    :return:Plot with close price, upperband, lowerband, entry and exit points
    '''
    ...
    plt.plot(plot_df['index_str'], plot_df['lowerband'], color='#7FFF7F', alpha=0.8, label='Lowerband')
    plt.plot(plot_df['index_str'], plot_df['upperband'], color='#FF7F7F', alpha=0.8, label='Upperband')
...
def plot_signals_with_supertrend_mcginley(df, entry, exit):
    '''
    :param df:DataFrame containing the price data.
    :param entry:List of entry points for trades.
    :param exit:List of exit points for trades.
    ...
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.title('Supertrend with McGinley Buy and Sell Signals')
    plt.legend()
