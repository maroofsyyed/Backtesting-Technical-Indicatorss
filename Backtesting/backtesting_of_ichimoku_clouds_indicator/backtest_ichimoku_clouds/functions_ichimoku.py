...
def calculate_performance_measures(entry, exit, roi, equity, risk_free_rate=None, annualize_coefficient=None):
    ...
    try:
        ...
        maximum = -1
    hit_ratio = len([trade for trade in exit if trade[1] > trade[0]]) / (trades * 100 + 1)  # Hit ratio
    expectancy = net_profit / (trades + 1)  # Expectancy
    ...
    sharpe_ratio = excess_return / (annualized_std_return * sqrt(len(equity)) + 1)
