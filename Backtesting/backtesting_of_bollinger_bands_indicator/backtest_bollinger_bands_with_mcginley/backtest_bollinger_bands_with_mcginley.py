'''
The backtest_bollinger_bands_with_mcginley function is a backtesting strategy that combines the Bollinger
Bands indicator with the McGinley Dynamic Average indicator. It aims to generate buy and sell signals
based on specific conditions derived from these Indicators.
'''

import math
from Indicators import bollinger_bands, mcginley_dynamic_average


def backtest_bollinger_bands_with_mcginley(data, investment, lookback=20, commission=5, share=0, stop_loss_pct=0.05):
    """
    Backtesting Bollinger Bands with McGinley Indicator Strategy
    ...
    # Initial conditions
    in_position = True if share > 0 else False
    equity = [investment]
    shares = share
    entry = []
    exit = []
    ...
    for i in range(1, len(data)):
        ...
        if not in_position and signals[i] == 1 and mcginley[i] > close[i]:
    ...
    print(f'Earning from investing $100k by backtest_bollinger_bands_with_mcginley is ${round(earning, 2)} (ROI = {roi}%)')
